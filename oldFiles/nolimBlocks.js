...
<script>
    $(document).ready(function () {
        var t838 = "#1111";
        var t838nolim = $('#1111.t-rec').addClass('nolim_forMenu');
        var searchIcon = $(".clsIcon");
        var idM = $('.clsIcon').parents('.t-rec');
        var idMh = $('.clsIcon').parents('.t-rec').children().children();
        idMh = $(idMh).css('height');
        $(t838).css('top', idMh);
        $('#1111').attr('nolim-search-state', '0');
        setTimeout(function () {
            if ($('.clsIcon').parents('.t-rec').css('position') == 'static') {
                idMh = $('.clsIcon').parents('.t-rec').children().children();
                idMh = $(idMh).height() + $('.clsIcon').parents('.t-rec').offset().top - 1 + 'px';
                $(t838).css('top', idMh);
        ...
        $(document).on('pointerup mouseup', (function (e) {
            if ($(t838).has(e.target).length === 0 && $(idM).has(e.target).length === 0 && !$(t838)
                .is(e.target)) {
                $(t838).fadeOut();
                $('[nolim-search-state]').attr('nolim-search-state', '0');
            }
